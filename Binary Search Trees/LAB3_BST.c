...
// nodes is 1 more than larger height of node's two subtrees.
{
    ...
    if (root != NULL) {
        maxHeight = 1 + max(height(root->leftChild), height(root->rightChild));
        return nodeheight;
...
// 
    {
        ...
        //If roots key is greater than k, we check to see if the leftchild has the key we want.
        if (root->key > k) {
            if (root->leftChild->key == k) return root;
            else
                return findParent(k,
                                  root->leftChild); //If the leftchild does not have the key we then look for the parent there.
        }
        //If the root is less than k, we look to see if the right child is node we are looking for
        if (root->key < k) {
            if (root->rightChild->key == k) return root; //If it is we return the root
            else
                return findParent(k, root->rightChild); //If it isn't we look for the parent at that point
        }
